...
glimpse(math_por)
# Answer: I see integers (i.e. numbers) and characters.
...
gather(alc) %>% glimpse
# Answer: gather() converts a multi-column table into a table of key-value pairs.
...
# Answer: There is a slight increase on average in absences among males with high alcohol consumption.
...
cbind(OR, CI)
# Answer: Sex has the widest interval.
#         All CIs (upper and lower) of target variables are >1.
#         Because ORs are also >1, there is a statistically significant association.
...
# Answer: I assume that with prob = 0, 30% of not high_use case predictions are incorrect.
#         Conversely with prob = 1, 70% of predicted high_use cases of incorrect.
#         Finally with prob = alc$probability, 23% of predicted high_use cases of incorrect.
...
cv$delta[1]
# Answer: The 10-fold prediction can be either lower or higher in repeated runs.
#         I assume it's because when we use K = nrow(alc), we cross validate each data point in their own test sets.
#         With K = 10, the data points are split randomly into 10 test sets (folds) in each run, therefore the error value changes.
